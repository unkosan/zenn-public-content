...
一応ですが、クラウドを運用する際は **MFA 設定** 行うの忘れずに。マイニング専用アカウントになってからでは遅いので。。。
MFA 設定はコンソールの右上から security settings から account protection を選択することで可能です。

![](/images/create-line-chatbot-with-alicloud-and-qwen/account-mfa.png)
